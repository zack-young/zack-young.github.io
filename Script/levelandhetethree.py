...
def GenoPhase (infile, bin_size, highun, lowun,
    ...
    for line in IN:
        ...
        while pos > end:
            if unconsis >= highun:
                print("\t".join(["high", "%s" % start, "%s" % end, "%s" % count, "%s" % homo, "%s" % hete, "%s" % unconsis]))
            elif lowun <= unconsis < highun:
                print("\t".join(["mid", "%s" % start, "%s" % end, "%s" % count, "%s" % homo, "%s" % hete, "%s" % unconsis]))
            else:
                print("\t".join(["low", "%s" % start, "%s" % end, "%s" % count, "%s" % homo, "%s" % hete, "%s" % unconsis]))
            start += bin_size
            end += bin_size
            count = 0
            consistent = 0
    ...
    if unconsis >= highun:
        ...
        print("\t".join(["low", "%s" % start, "%s" % end, "%s" % count, "%s" % homo, "%s" % hete, "%s" % unconsis]))        
        #  print("\t".join([b, "%s" % start, "%s" % end, "%s" % count, "%s" % count1, "%s" % count2, "%s" % counthete, "%s" % unconsis]))
    #
    if infile:
        IN.close()
    #
