...
def Makelis(chromsome):
    global list1
    list1 = []
    f = open("/data/user/yangzz/mapping/S15/ReseqPipeNew/09.filterVCF/GT_AD/MASK/T123_mask/chr" + chromsome + ".mask_filtered_combine", "r")
    mask = f.readlines()
    for line in mask:
...
#    print(list1)
    ...
    for line in IN:
        ...
        b = chr_cur[0]
        while pos > end:
            asx = math.log10(2*father + hete + 1)
            asy = math.log10(2*mother + hete + 1)
            START = str(start)
# shield CNV in 14;55
           # if START in list1:
           #     pass
           # else:
           # if asy > asx*0.5+1.3:
           #     print("\t".join([b, "%s" % start, "%s" % end, "%s" % hete, "%s" % father, "%s" % mother, "nd3097"]))
           # elif asy < asx*1.7-1.9:
           #     print("\t".join([b, "%s" % start, "%s" % end, "%s" % hete, "%s" % father, "%s" % mother, "lx987"]))
           # else:
           #     print("\t".join([b, "%s" % start, "%s" % end, "%s" % hete, "%s" % father, "%s" % mother, "undefined"]))
             # #
            if asy > asx*1.1-0.3:
                print("\t".join([b, "%s" % start, "%s" % end, "%s" % hete, "%s" % father, "%s" % mother, "nd3097"]))
            else:
...
#                print("\t".join([b, "%s" % start, "%s" % end, "%s" % hete, "%s" % father, "%s" % mother, "nd3097"]))
        ...
        if IsGeno(gs):
            son += 1
        if Isunconsistent(g1, g2) and IsGeno(gs) and IsGeno(g1) and IsGeno(g2):
            if IsHete(gs):
                hete += 1
            else:
                if g1 == gs:
                ...
                if g2 == gs:
                    mother += 1
                if g1 != gs and g2 != gs:       #elif 就是 else
                    undefinedpar += 1
