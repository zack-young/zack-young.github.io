...
def GenoPhase(infile, bin_size,maxun, lowun, chr_col=1, pos_col=2, ref_col=3, alt_col=4, gt_col=5, p2_col=4, s_col=5, sample='S1'):
    # open the infile
    try:
        if infile:
...
#        continue
        ...
        while pos > end:                        
            ...
            if float(dic1[sample][(num)*3+ch][count]) >= 0.6:
                pass   
#                if lowun < DP_ratio < maxun:
#                   pass
#                else:
#                   print("\t".join([b, art,"%s" % start, "%s" % end]))
            else:
                print("\t".join([b, art,"%s" % start, "%s" % end]))
            count1 = 0
            count2 = 0
...
def main():
    ...
    if (options.outfile is not None) :  #??????
        ...
        #
    #
    GenoPhase(options.infile, int(options.bin_size),float(options.maxun),float(options.lowun), int(options.chr_col), int(options.pos_col),
