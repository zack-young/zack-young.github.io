...
#    continue
    ...
    for line in IN:
        ...
        while pos > end:
            START = str(start)
            deli = abs(count3-count2)
            if START in list1:
                pass
            else:
                if deli > 3140:
                    print("\t".join([b, "%s" % start, "%s" % end, "%s" % deli, "level1"]))
                elif deli <= 796:
                    print("\t".join([b, "%s" % start, "%s" % end, "%s" % deli, "level4"]))
                elif 796 < deli <= 1589:
                    print("\t".join([b, "%s" % start, "%s" % end, "%s" % deli, "level3"]))
                else:
                    print("\t".join([b, "%s" % start, "%s" % end, "%s" % deli, "level2"]))
            # #
            start += bin_size
            end += bin_size
            count1 = 0
