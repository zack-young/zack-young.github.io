...
def Makedic(sample):
    ...
    for j in range(0, 7):
        for i in range(0, 3):
            lett = ["A", "B", "D"]
            dic1[sample].append([])
            f = open("/data2/rawdata2/readDepth/" + sample + "/chr" + str(j+1) + lett[i] + ".1M.norm", "r")
            CNVT = f.readlines()
            #lis = []
            for line in CNVT:
            #     lis.append(line)
...
def GenoPhase(infile, bin_size, chr_col=1, pos_col=2, ref_col=3, alt_col=4, gt_col=5, p2_col=4, s_col=5, sample='S1'):
    # open the infile
    try:
        if infile:
...
#            print("\t".join([b, "%s" % start, "%s" % end, "%s" % hete, "%s" % father, "%s" % mother]))
            ...
            if float(dic1[sample][(num)*3+ch][count]) >= 0.6:
                print("\t".join([b, art + ".DP", "%s" % start, "%s" % end, "%s" % count1, "%s" % count2,
                                 "%.2f" % ratio, "RATIO", "%.2f" % DP_ratio, "%s" % count3]))  
                print("\t".join([b, art, "%s" % start, "%s" % end, "%s" % count1, "%s" % count2,
                                 "%.2f" % DP_ratio, "DP", "%.2f" % ratio, "%s" % count3]))
            else:
                print("\t".join([b, art + ".DP", "%s" % start, "%s" % end, "%s" % count1, "%s" % count2,
                                 "-0.1", "RATIO", "%.2f" % DP_ratio, "%s" % count3]))
                print("\t".join([b, art, "%s" % start, "%s" % end, "%s" % count1, "%s" % count2,
                                 "%.2f" % DP_ratio, "DP", "-0.1", "%s" % count3]))
