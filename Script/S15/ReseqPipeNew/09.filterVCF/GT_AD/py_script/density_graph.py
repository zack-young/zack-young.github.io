...
def GenoPhase(infile, bin_size, levela=1, levelb=1, levelc=1, level4=1,
    ...
    for line in IN:
        ...
        while pos > end:
            ...
            if START in list1:
                ...
                if count3 >= levela:
                    print("\t".join([b, "%s" % start, "%s" % end, "%.3f" % ratio, "%s" % count3, "%s" % count1, "level1"]))
                elif count3 < levelc:
                    print("\t".join([b, "%s" % start, "%s" % end, "%.3f" % ratio, "%s" % count3, "%s" % count1, "level4"]))
                elif levelc <= count3 < levelb:
                    print("\t".join([b, "%s" % start, "%s" % end, "%.3f" % ratio, "%s" % count3, "%s" % count1, "level3"]))
                else:
                    print("\t".join([b, "%s" % start, "%s" % end, "%.3f" % ratio, "%s" % count3, "%s" % count1, "level2"]))
            # #
            start += bin_size
            end += bin_size
            count1 = 0
...
def main():
    ...
    if (options.outfile is not None) :  #??????
        ...
        #
    #
    GenoPhase(options.infile, int(options.bin_size), int(options.levela), int(options.levelb), 
              int(options.levelc), int(options.level4),
