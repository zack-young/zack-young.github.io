...
for a in {${SAMPLE1},${SAMPLE2}}; do
    if [[ ! -d "field_cultivar/${a}" ]]&&[[ ! -d "CNV_heatmap/${a}_${time_date}" ]]; then 
              ...
              cat CNV_heatmap/${a}_${time_date}/chr*.1M.tmp > CNV_heatmap/${a}_${time_date}/combine_1M_DP
              sh /data/user/yangzz/mapping/09.filterVCF/GT_AD/field_cultivar/script_1M.sh CNV_heatmap/${a}_${time_date} &
    fi 
done
wait
for num in {1..7}; do
    for i in {A,B,D}; do
        ...
        for sample in {${SAMPLE1},${SAMPLE2}}; do
                 ...
                 if [[ -d "field_cultivar/${sample}" ]]&&[[ -s "field_cultivar/${sample}/chr${num}${i}.1M.norm" ]]; then
                      ./muti_func_snp_compare.py -b 1000000 -i field_cultivar/${sample}/chr${num}${i}.1M.norm --mask_cnv on -o tmp/${sample}_${time_date}/chr${num}${i}.mask_CNV 
                 else
...
#            ./muti_func_snp_filter.py -i ${SAMPLE1}_${SAMPLE2}/chr${num}${i}_snp.gt --single_DP_GQ on -s ${first} --chromsome tmp/${SAMPLE1}_${time_date}/chr${num}${i}.mask_CNV |grep ${item}|sort -nk1,1> tmp/${SAMPLE1}_${time_date}/chr${num}${i}_snp_${item} &
#            fi
#            if [[ ! -s "tmp/${SAMPLE2}_${time_date}/chr${num}${i}_snp_${item}" ]]; then
#            ./muti_func_snp_filter.py -i ${SAMPLE1}_${SAMPLE2}/chr${num}${i}_snp.gt --single_DP_GQ on -s ${second} --chromsome tmp/${SAMPLE2}_${time_date}/chr${num}${i}.mask_CNV |grep ${item}|sort -nk1,1> tmp/${SAMPLE2}_${time_date}/chr${num}${i}_snp_${item} &
#            fi
#            if [[ ! -s "tmp/${SAMPLE1}_${time_date}/chr${num}${i}_indel_${item}" ]]; then
#            ./muti_func_snp_filter.py -i ${SAMPLE1}_${SAMPLE2}/chr${num}${i}_indel.gt --single_DP_GQ on -s ${first} --chromsome tmp/${SAMPLE1}_${time_date}/chr${num}${i}.mask_CNV |grep ${item}|sort -nk1,1> tmp/${SAMPLE1}_${time_date}/chr${num}${i}_indel_${item} &   
#            fi
#            if [[ ! -s "tmp/${SAMPLE2}_${time_date}/chr${num}${i}_indel_${item}" ]]; then
#            ./muti_func_snp_filter.py -i ${SAMPLE1}_${SAMPLE2}/chr${num}${i}_indel.gt --single_DP_GQ on -s ${second} --chromsome tmp/${SAMPLE2}_${time_date}/chr${num}${i}.mask_CNV |grep ${item}|sort -nk1,1> tmp/${SAMPLE2}_${time_date}/chr${num}${i}_indel_${item} & 
#            fi
#            wait_all
#        done
#    done
