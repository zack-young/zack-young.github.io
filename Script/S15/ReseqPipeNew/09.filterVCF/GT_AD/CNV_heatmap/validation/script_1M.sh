...
#  gawk '{sum+=$4} (NR%1000)==0{print sum/1000; sum=0} END{n=(NR%1000);if(n!=0){print sum/n}}' /data2/rawdata2/readDepth_ori_1k/$SM/${chr}.2.1k.DP > $SM/${chr}.2.1M.tmp
    gawk '{sum+=$4} (NR%1000)==0{print sum/1000; sum=0} END{n=(NR%1000);if(n!=0){print sum/n}}' $1/${chr}.1.1k.DP > $1/${chr}.1.1M.tmp
    gawk '{sum+=$4} (NR%1000)==0{print sum/1000; sum=0} END{n=(NR%1000);if(n!=0){print sum/n}}' $1/${chr}.2.1k.DP > $1/${chr}.2.1M.tmp
done
