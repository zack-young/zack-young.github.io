...
def Count(infile,count,compensent_deletion,compensent_duplication):
    # open the infile
    try:
        if infile:
    ...
    if count == 'on' :
        ...
        for key,value in dic1.items():
            print('{key}\t{key2}\t{value}'.format(key=key, key2=int(key)+1000000, value=value/80), sep='\t')
...
def main():
    ...
    if (options.outfile is not None) :  #??????
        ...
        #
    #
    Count(infile=options.infile, count=options.count, compensent_deletion=options.compensent_deletion, compensent_duplication=options.compensent_duplication)
    #
#
