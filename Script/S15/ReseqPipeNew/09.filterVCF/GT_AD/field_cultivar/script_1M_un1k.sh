...
ave=`/data/user/yangzz/mapping/09.filterVCF/GT_AD/field_cultivar/count_CNV.py --normalize on -i ${1}/combine_1M_DP`
...
for CHR in chr1A chr1B chr1D chr2A chr2B chr2D chr3A chr3B chr3D chr4A chr4B chr4D chr5A chr5B chr5D chr6A chr6B chr6D chr7A chr7B chr7D;do
  gawk -vOFS="\t" -vave=$ave '{print $1/ave}' ${1}/${CHR}.1.1M.tmp > ${1}/${CHR}.1M.norm_un1k
  gawk -vOFS="\t" -vave=$ave '{print $1/ave}' ${1}/${CHR}.2.1M.tmp >> ${1}/${CHR}.1M.norm_un1k
