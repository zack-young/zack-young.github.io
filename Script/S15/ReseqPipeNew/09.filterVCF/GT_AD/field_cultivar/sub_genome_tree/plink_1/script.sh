...
for CHR in chr1A chr1B chr2A chr2B chr3A chr3B chr4A chr4B chr5A chr5B chr6A chr6B chr7A chr7B chr1D chr2D chr3D chr4D chr5D chr6D chr7D;do
  /data/user/yangzz/worktools/Plink/plink --bcf ~/mapping/08.mergeGVCF/field_cultivar/${CHR}.trimed.snp.bcf.gz --make-bed --allow-extra-chr --out ${CHR}_setname --set-missing-var-ids @:# &
  wait_all
done
wait
