...
ave=`/data/user/yangzz/mapping/09.filterVCF/GT_AD/field_cultivar/count_CNV.py --normalize on -i combine_90_1M_DP`
for CHR in chr1A chr1B chr1D chr2A chr2B chr2D chr3A chr3B chr3D chr4A chr4B chr4D chr5A chr5B chr5D chr6A chr6B chr6D chr7A chr7B chr7D;do
  gawk -vOFS="\t" -vave=$ave '{print $1/ave}' ${CHR}.1.1M.90_tmp > ${CHR}.1M.90_norm
  gawk -vOFS="\t" -vave=$ave '{print $1/ave}' ${CHR}.2.1M.90_tmp >> ${CHR}.1M.90_norm
done
