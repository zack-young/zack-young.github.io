...
# distF <- '/data/user/yangzz/mapping/09.filterVCF/GT_AD/field_cultivar/sub_genome_tree/plink_2/AABBDD_setname_maf003_miss2.mdist'
# sampleF <- '/data/user/yangzz/mapping/09.filterVCF/GT_AD/field_cultivar/cultivar_list'
# annoF <- '/data/user/yangzz/mapping/09.filterVCF/GT_AD/field_cultivar/metadata_cultivar_nogroup.txt'
# sampleP <- '/data/user/yangzz/mapping/09.filterVCF/GT_AD/field_cultivar/cultivar_list'
...
# out <- 'AABB_setname_maf003_miss05'
# 
# 
...
tmp_index <- rownames(dist_mat) %in% meta_data$Sample[match(present_sample[[1]], meta_data$Sample)] # match return the position of x in table
dist_mat <- dist_mat[tmp_index, tmp_index]

sample_present <- meta_data[meta_data$Sample %in% rownames(dist_mat),]

LIST <- sample_present$label
names(LIST) <- sample_present$Sample #assign name to LIST
rownames(dist_mat) = LIST[rownames(dist_mat)]

dis_matrix <- as.dist(dist_mat) #dist try to caculate diatance; as.dist try to coerce a object to a distance matrix
...
if(!plotT){
  ...
  plot(NJ_tree, type = "u", show.tip.label = T, edge.color = col.vector, cex=0.2)  #plot.phylo (ape)
