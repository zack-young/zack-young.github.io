...
def GenoPhase(infile, bin_size, highun, lowun, chr_col=1, pos_col=2, ref_col=3, alt_col=4, gt_col=5, p2_col=4, s_col=5, sample='S1'):
    # open the infile
    try:
        if infile:
...
#            if float(dic1[sample][(num)*3+ch][count]) >= 0.6 and lowun <= DP_ratio <= highun:
            #if count3 >= 2000:
            print("\t".join([b, art, "%s" % start, "%s" % end, "%s" % count3,"%s" % count2, "high"]))  
            #else:
            #    print("\t".join([b, art, "%s" % start, "%s" % end, "%s" % count3,"%s" % count2, "low"]))
            #print("\t".join([b, art, "%s" % start, "%s" % end, "%s" % count1, "%s" % count2,
            #                 "%.4f" % ratio, "%.2f" % DP_ratio, "%s" % count3]))
...
def main():
    ...
    if (options.outfile is not None) :  #??????
        ...
        #
    #
    GenoPhase(options.infile, int(options.bin_size),str(options.highun),str(options.lowun),
