...
echo $lines|tr ' ' '\n' | awk -F '\t' 'NR==FNR{a[$1]=$1;next}NR>FNR{if($4"_"$3 in a ==1) print $0}' -  ~/mapping/fieldergenomecompare/metadata_cultivar_196_nongke_10+.txt | cut -f2 > use_samplelist_rht4B${num}_mcl
num=$(( $num +1 ))
fi
done < ~/mapping/fieldergenomecompare/statistic/GSR_MCL/ALL_mcl_dir/chr4B.31_mcl
#while read line;do
