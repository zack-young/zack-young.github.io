...
# input file
infile <- "/data/user/yangzz/mapping/09.filterVCF/GT_AD/snp_heatmap/Rht4B_SNP_2.txt" 
tmp <- read.table(infile, header = T, check.names = F, sep = "\t", stringsAsFactors = F)

# rht_4D <- "/data/user/yangzz/mapping/09.filterVCF/GT_AD/snp_heatmap/metadata_cultivar_rht4B.txt" 
# rht_4D_list <- read.table(rht_4D, header = T, check.names = F, sep = "\t", stringsAsFactors = F)
cn <- colnames(tmp)[-1]
rn <- as.character(tmp$ANN)
tmp <- tmp[-1]
#sp_pos <- match(rht_4D_list$Name,cn)
lis <- c(cn[sp_pos],cn[-sp_pos])
tmp <- tmp[,lis]
cn <- lis
# lis <- c()
# for (i in seq(1,nrow(tmp))){
#   if (length(which(tmp[i,]==0.5))/ncol(tmp)>0.3) {
#     lis <- c(lis,i)
#   }
# }
# tmp <- tmp[-lis,]
...
# group file
groupf <- read.table("/data/user/yangzz/mapping/09.filterVCF/GT_AD/snp_heatmap/tmp.4B", col.names = c("Sample", "Group"), sep = "\t", stringsAsFactors = F)
