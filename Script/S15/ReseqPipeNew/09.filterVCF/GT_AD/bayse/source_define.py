...
#    print(list1)
    ...
    for line in IN:
        ...
        g1 = tokens[p1_col - 1]
        dp1 = int(tokens[p1_col])
        g2 = tokens[p2_col - 1]
        dp2 = int(tokens[p2_col])
        gs = tokens[s_col - 1]
        dps = int(tokens[s_col])
        ...
        while pos > end:
            ...
            if START in list1:
                ...
                print("\t".join([b, "%s" % start, "%s" % end, "%s" % hete, "%s" % father, "%s" % mother, "%s" % undefinedpar]))
            # #
            start += bin_size
            end += bin_size
            count1 = 0
            ...
            # compare genotype
        count1 += 1
        if 5 <= dp1 <= 7 and 5 <= dp2 <= 7 and 5 <= dps <= 7:
            if Isunconsistent(g1, g2) and IsGeno(gs):
                if IsGeno(g1) or IsGeno(g2):
                    if IsHete(gs):
                        hete += 1
                    else:
                        if g1 == gs:
