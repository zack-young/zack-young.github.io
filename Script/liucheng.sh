...
#./raw_data_wash.py -i /data/user/yangzz/mapping/5xresult/combine5M -o /data/user/yangzz/mapping/test/

for VCF in /data/user/yangzz/mapping/5xresult/chr*.vcf.GT.filter; do
  CHR=`echo $VCF | sed s/.vcf.GT.filter//g | uniq`
#  ./lastgeno.py -b 5000000 -i ${VCF} -u 0.42 -1 5 -2 7 -s 6 -o ${CHR}.find.filter &
  ./lastgeno.py -b 1000000 -i ${VCF} -m 708 -l 50 -d 0.00092 -1 5 -2 7 -s 6 -o ${CHR}.find.filter &
...
#for BCF in /data2/rawdata2/mergeFile/10.concatVCF/chr??.snp.bcf.gz; do
#  CHR=`basename ${BCF} | sed s/.snp.bcf.gz//g`
#  bcftools view -s S1,S2,S3,S4 ${BCF} -Oz -o /data/user/yangzz/mapping/${CHR}.raw.vcf
#done

#bcftools query -l /data2/rawdata2/mergeFile/10.concatVCF/chr3A.2.snp.bcf.gz

for BCF in /data2/rawdata2/mergeFile/10.concatVCF/chr??.snp.bcf.gz; do
  CHR=`basename ${BCF} | sed s/.snp.bcf.gz//g`
#  bcftools view -s LX987,S140,s3097-1 ${BCF} -Oz -o /data/user/yangzz/mapping/5xresult/${CHR}.raw.bcf &
done
