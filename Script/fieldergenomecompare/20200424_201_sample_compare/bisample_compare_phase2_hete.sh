         ...
         if [ "${SAMPLE1}" != "${SAMPLE2}" ]; then
           ...
           for num in {1..7}; do
             for i in {A,B,D}; do
                 ...
                 if [ "${b}" == 0 ];then
                   awk '{print $0}'   ${SAMPLE1}_${SAMPLE2}/chr${num}${i}.1M.hete_density  > ${SAMPLE1}_${SAMPLE2}/chr${num}${i}.1M.delcnv_hete_density &
                 else
                   awk 'NR==FNR{a[$2]=$2;next}NR>FNR{if($2 in a ==0) print $0}'  ${DEV_PATH}/${SAMPLE1}_${SAMPLE2}/chr${num}${i}.${SAMPLE1}to${SAMPLE2}all_CNV  ${SAMPLE1}_${SAMPLE2}/chr${num}${i}.1M.hete_density  > ${SAMPLE1}_${SAMPLE2}/chr${num}${i}.1M.delcnv_hete_density &
                 fi
        #
             done
           done
