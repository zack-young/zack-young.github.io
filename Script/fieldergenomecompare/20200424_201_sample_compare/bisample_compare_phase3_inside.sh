...
for chr in chr1A chr2A chr3A chr4A chr5A chr6A chr7A chr1B chr2B chr3B chr4B chr5B chr6B chr7B chr1D chr2D chr3D chr4D chr5D chr6D chr7D; do
    ./muti_func_snp_compare.py --two_diff_level on -i ${SAMPLE1}_${SAMPLE2}/${chr}.1M.delcnv_density --chromosome ${chr} --sample1 ${SAMPLE1} --sample2 ${SAMPLE2} 
    cat ${SAMPLE1}_${SAMPLE2}/${chr}.only_homo_snp_undefined_level ${dev}/${SAMPLE1}_${SAMPLE2}/${chr}.${SAMPLE1}to${SAMPLE2}all_CNV > ${SAMPLE1}_${SAMPLE2}/${chr}.homo_undefined_snp_level
done
         #wait
