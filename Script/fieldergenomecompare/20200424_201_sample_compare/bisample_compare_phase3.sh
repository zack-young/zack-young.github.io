...
for SAMPLE1 in ${arr[@]};do
    ...
    for SAMPLE2 in ${arr_2[@]};do
         ...
         #./muti_func_snp_compare.py --two_diff_level on -i ${SAMPLE1}_${SAMPLE2}/${chr}.1M.hete_miss_delcnv_density --sample1 ${SAMPLE1} --sample2 ${SAMPLE2}
         #cat /data/user/shinyug/HMM_for_yzz_comp/200802/201_sample_compare/${SAMPLE1}_${SAMPLE2}/${chr}.only_homo_snp_hmm_level ${DEV_PATH}/${SAMPLE1}_${SAMPLE2}/${chr}.${SAMPLE1}to${SAMPLE2}all_CNV > ${SAMPLE1}_${SAMPLE2}/${chr}.homo_hmm_snp_level
         cat ${SAMPLE1}_${SAMPLE2}/${chr}.only_homo_snp_undefined_level ${DEV_PATH}/${SAMPLE1}_${SAMPLE2}/${chr}.${SAMPLE1}to${SAMPLE2}all_CNV > ${SAMPLE1}_${SAMPLE2}/${chr}.homo_undefined_snp_level
         done)&
         else
         (for chr in chr1A chr2A chr3A chr4A chr5A chr6A chr7A chr1B chr2B chr3B chr4B chr5B chr6B chr7B chr1D chr2D chr3D chr4D chr5D chr6D chr7D; do
         #./muti_func_snp_compare.py --two_diff_level on -i ${SAMPLE2}_${SAMPLE1}/${chr}.1M.delcnv_density --sample1 ${SAMPLE2} --sample2 ${SAMPLE1}
         cat ${SAMPLE2}_${SAMPLE1}/${chr}.only_homo_snp_undefined_level ${DEV_PATH}/${SAMPLE2}_${SAMPLE1}/${chr}.${SAMPLE2}to${SAMPLE1}all_CNV > ${SAMPLE2}_${SAMPLE1}/${chr}.homo_undefined_snp_level
         done)&
         fi
         #(#./ptf_maker.py -p "/data/user/shinyug/HMM_for_yzz_comp/whole_samples_raw/201_sample_compare/${SAMPLE1}_${SAMPLE2}" -s ".homo_hmmed_snp_level" > plotfile/plotfile_${SAMPLE1}_${SAMPLE2}_undefined
         # ./ptf_maker.py -p "/data/user/yangzz/mapping/fieldergenomecompare/20200424_201_sample_compare/${SAMPLE1}_${SAMPLE2}" -s ".homo_snp_level" > plotfile/plotfile_${SAMPLE1}_${SAMPLE2}_undefined
         #Rscript ~/R/graph_diff_sim.R -d ~/mapping/fieldergenomecompare/20200424_201_sample_compare --sample1 ${SAMPLE1} --sample1_name ${arr_3[${SAMPLE1}]} --sample2 ${SAMPLE2} --sample2_name ${arr_3[${SAMPLE2}]} -s "bayes" ) &
