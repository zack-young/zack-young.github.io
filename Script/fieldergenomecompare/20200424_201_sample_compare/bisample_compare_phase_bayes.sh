...
for SAMPLE1 in ${arr[@]};do
    ...
    for SAMPLE2 in ${arr_2[@]};do
         ...
         num_tmp=$(( $num_tmp + 1 ))
         echo $num_tmp
         cp /data/user/shinyug/HMM_for_yzz_comp/whole_samples_raw/201_sample_compare/${SAMPLE1}_${SAMPLE2}/*.homo_hmmed_snp_level ${SAMPLE1}_${SAMPLE2} &
...
#         sed 1d ${SAMPLE1}_${SAMPLE2}/${chr}.1M.delcnv_density |paste ${SAMPLE1}_${SAMPLE2}/${chr}.only_homo_snp_level ${SAMPLE1}_${SAMPLE2}/${chr}.only_homo_snp_undefined_level -|cut -f1,2,3,4,8,12|awk '{if($4 != $5) print $0 }' >> ../statistic/bayes/${chr}_bayes)&
         #done)&
         #cat ${SAMPLE1}_${SAMPLE2}/chr*{A,B}.only_homo_snp_undefined_level > ../statistic/bayes/AB_${SAMPLE1}_${SAMPLE2} &
         #cat ${SAMPLE1}_${SAMPLE2}/chr*D.only_homo_snp_undefined_level > ../statistic/bayes/D_${SAMPLE1}_${SAMPLE2} &
         #(for chr in chr1A chr2A chr3A chr4A chr5A chr6A chr7A chr1B chr2B chr3B chr4B chr5B chr6B chr7B chr1D chr2D chr3D chr4D chr5D chr6D chr7D; do
         #sed 1d ${SAMPLE1}_${SAMPLE2}/${chr}.1M.delcnv_density |paste ${SAMPLE1}_${SAMPLE2}/${chr}.only_homo_snp_level ${SAMPLE1}_${SAMPLE2}/${chr}.only_homo_snp_undefined_level -|cut -f1,2,3,4,8,12|awk '{if($4 != $5) print $0 }' > ../statistic/bayes/${chr}_bayes_${SAMPLE1}_${SAMPLE2}
         #done
         #)&
         fi
