...
#    Makedic(sample)
    ...
    if two_diff_level == 'on':
        ...
        if count > 1:
            ...
            #dic={22 :[ 0.04503934 , 0.005408375 ],23 :[ 0.0414033 , 0.006445105 ],24 :[ 0.03814287 , 0.007600913 ],25 :[ 0.03521064 , 0.008879483 ],26 :[ 0.03256623 , 0.0102839 ],27 :[ 0.03017515 , 0.01181669 ],28 :[ 0.02800778 , 0.0134798 ],29 :[ 0.0260386 , 0.01527467 ],30 :[ 0.0242455 , 0.01720223 ],31 :[ 0.02260931 , 0.01926292 ],32 :[ 0.0211133 , 0.02145677 ],33 :[ 0.01974285 , 0.02378338 ],34 :[ 0.01848514 , 0.02624196 ],35 :[ 0.01732889 , 0.02883139 ],36 :[ 0.01626416 , 0.03155022 ],37 :[ 0.01528214 , 0.03439672 ],38 :[ 0.01437502 , 0.03736888 ],39 :[ 0.01353588 , 0.04046447 ],40 :[ 0.01275854 , 0.04368105 ],41 :[ 0.01203747 , 0.04701598 ],42 :[ 0.01136773 , 0.05046648 ],43 :[ 0.01074491 , 0.0540296 ],44 :[ 0.01016501 , 0.0577023 ],45 :[ 0.009624462 , 0.06148142 ],46 :[ 0.009120027 , 0.06536372 ],47 :[ 0.008648789 , 0.06934589 ],48 :[ 0.008208106 , 0.07342456 ]}
            frame.loc[(1000 < frame[3]) ,5] = 'high'
            frame.loc[(10 >= frame[3]) ,5] = 'low'
            frame.loc[(1000 >= frame[3])&(10 < frame[3]) ,5] = 'mid'
            #frame.loc[(48 > frame[3])&(22 <= frame[3]) ,5] = 'undefined'
            #for num in range(22,48):
            #    for i in frame.loc[(num == frame[3]) ,1]:
            #        bayes_frame = frame.loc[((i-5000000) <= frame[1])&(frame[1] <= (i+6000000))&(frame[1] != i),]
            #        if bayes_frame[(bayes_frame[5]=='mid')].empty :
            #            len_diff = 0
            #        else:
            #            len_diff = len(bayes_frame[(bayes_frame[5]=='mid')])
            #        if bayes_frame[(bayes_frame[5]=='low')].empty :
            #            len_same = 0
            #        else:
            #            len_same = len(bayes_frame[(bayes_frame[5]=='low')])
            #        if len_diff*dic[num][1] > len_same*dic[num][0]:
            #            frame.loc[(frame[1] == i),5]='mid'
            #        else:
            #            frame.loc[(frame[1] == i),5]='low'
            ...
            diff_file =  sample1 + '_' +sample2 + "/" + chr + ".only_homo_snp_undefined_level"#".only_homo_snp_hete_miss_undefined_level"#".only_homo_snp_level"
            frame.to_csv(diff_file,sep='\t',na_rep='Na',float_format='%.1f', columns=[0,1,2,5], header=False,index=False)
        else:
            diff_file = open(sample1 + '_' +sample2 + "/" + chr + ".only_homo_snp_undefined_level",'w')
            diff_file.close()
    ...
    if mask_cnv == 'on':
        ...
        for line in readline:
            ...
            if RD < 0.5:                
                print(filename3,start,end,"ownCNV_deletion",sep="\t")
            if RD > 1.3:                
                print(filename3,start,end,"ownCNV_duplication",sep="\t")
            start += bin_size
            end += bin_size
    #
    if cnv_region == 'on':
