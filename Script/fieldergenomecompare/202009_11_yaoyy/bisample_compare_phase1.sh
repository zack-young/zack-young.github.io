...
path_local="/data/user/yangzz/mapping/09.filterVCF/GT_AD"
  #/data2/public/ReseqData/workflowFile/
...
for SAMPLE1 in ${arr[@]};do
    (if [[ ! -d "${SAMPLE1}" ]]; then
         ...
         bedtools coverage -a ~/mapping/09.filterVCF/GT_AD/CNV_heatmap/bin/bed/${chr}.1.1k.bed -b ${DIR}/${SAMPLE1}/05.mergeAsplit/${chr}.*.bam -counts -sorted > ${SAMPLE1}/${chr}.1.1k.DP
         bedtools coverage -a ~/mapping/09.filterVCF/GT_AD/CNV_heatmap/bin/bed/${chr}.2.1k.bed -b ${DIR}/${SAMPLE1}/05.mergeAsplit/${chr}.*.bam -counts -sorted > ${SAMPLE1}/${chr}.2.1k.DP

         gawk '{sum+=$4} (NR%1000)==0{print sum/1000; sum=0} END{n=(NR%1000);if(n!=0){print sum/n}}' ${SAMPLE1}/${chr}.1.1k.DP > ${SAMPLE1}/${chr}.1.1M.tmp 
         gawk '{sum+=$4} (NR%1000)==0{print sum/1000; sum=0} END{n=(NR%1000);if(n!=0){print sum/n}}' ${SAMPLE1}/${chr}.2.1k.DP > ${SAMPLE1}/${chr}.2.1M.tmp 
         done 
