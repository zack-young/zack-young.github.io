...
sample=`echo $line | awk -F '[ _,]' '{if ($4=="CDC"||$4=="SY")print $4"_"$5 ;else print $4}'`
echo $chr $start
...
awk -vi=${i} '{if ($11==i) print $0}' ${file_name}_maf_alt_block.txt | sort -k3,3 >${file_name}tmp.txt

cut -f3  ${file_name}tmp.txt| uniq | awk -vnum=$num '{print $1"\t"NR+num}' > ${file_name}test.txt
line=`cat ${file_name}test.txt|wc -l `
num=$(( $num + $line ))
echo $num
awk 'BEGIN{OFS="\t"}NR==FNR{a[$1]=$2;next} NR!=FNR{if($3 in a ==1) print $11,$1,$2,$3,a[$3],$4,$5,$6,$7,$8,$9,$10}' ${file_name}test.txt ${file_name}tmp.txt >> ${file_name}_final.txt
