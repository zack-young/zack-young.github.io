awk '{print $1"\t"int($2/1000000)*1000000+1"\t"int($2/1000000)*1000000+2"\t"$4}' gene_list_all.txt|sort -k1,1 -k2,2n | bedtools merge -i - -c 4 -o distinct > gene_list_all_merge.txt
...
cat gene_list_hap_type.txt|grep -Ev 'deletion|duplication'|sort -k1,1 -k2,2n > gene_list_hap_type_good_count_filter.txt
