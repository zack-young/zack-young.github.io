...
SAMPLE2=`echo $line|awk -F ',' '{print $2}'`
    ./muti_func_snp_compare.py --two_diff_level on -i dev/${SAMPLE1}_${SAMPLE2}/${chr}.1M.density --sample1 ${SAMPLE1} --sample2 ${SAMPLE2} --chromosome ${chr}
    line=`cat dev/${SAMPLE1}_${SAMPLE2}/${chr}.${SAMPLE1}to${SAMPLE2}all_CNV|wc -l`
    if test $line = 0;then
    cat dev/${SAMPLE1}_${SAMPLE2}/${chr}.only_homo_snp_undefined_level  > dev/${SAMPLE1}_${SAMPLE2}/${chr}.homo_undefined_snp_level
    else
