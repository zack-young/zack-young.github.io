...
# sum bin to 100k
SM=$1
path_SM="/data/user/yangzz/mapping/fieldergenomecompare/${1}_2020-04-17162314"
mkdir ${1}
for chr in chr1A chr1B chr1D chr2A chr2B chr2D chr3A chr3B chr3D chr4A chr4B chr4D chr5A chr5B chr5D chr6A chr6B chr6D chr7A chr7B chr7D;do
    gawk '{sum+=$4} (NR%10)==0{print sum/10; sum=0} END{n=(NR%10);if(n!=0){print sum/n}}' /data2/rawdata2/readDepth_ori_1k/$SM/${chr}.1.1k.DP > $SM/${chr}.1.1M.tmp
