...
def Count(infile, combine_data, count, ratio,compensent, normalize, split, chrom, pop_num, compensent_duplication):
    # open the infile
    try:
        if infile:
    ...
    if count == 'on' :
        ...
        for key,value in dic1.items():
            print('{key}\t{key2}\t{value}'.format(key=key, key2=int(key)+1000000, value=int(value)), sep='\t')  #value=value/int(pop_num))
    if ratio == 'on' :
        for line in IN: 
            line = line.strip().split('\t')
            dic1[line[1]] = dic1.setdefault(line[1], 0)+1
        for key,value in dic1.items():
            print('{key}\t{key2}\t{value}'.format(key=key, key2=int(key)+1000000, value=value/int(pop_num)), sep='\t')  #value=value/int(pop_num))
...
#
    if normalize == 'on':
        ...
        for line in IN:
            item = line.strip("\n").split("\t")
            if item[0] != '0':
                lis.append(round(float(item[0]),2))
    ...
    if compensent == 'on' :
        ...
        loc = num + ch
        if '1' in dic1:
            use_num = str(dic1["1"][1])
            print("\t".join(['1', '1000001', "%s" % use_num]))
        else:
            print("\t".join(['1', '1000001', "0"]))
        for item in range(1, int(wholen[loc]/1000000)+1):            
            ...
            if item in dic1:
                ...
                print("\t".join(["%s" % item, "%s" % str(int(item)+1000000), "0"]))
...
def main():
    ...
    if (options.outfile is not None) :  #??????
        ...
        #
    #
    Count(infile=options.infile, combine_data=options.combine_data, count=options.count,ratio=options.ratio, chrom=options.chrom,
          normalize=options.normalize, compensent=options.compensent, split=options.split,
