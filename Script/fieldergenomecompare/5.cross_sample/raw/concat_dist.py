...
def concat_dist(CHR="chr1A", BINSIZE=5, THR=1000, OUTFILE="test"):
    #
    BED_PATH="/data/user/yangzz/mapping/fieldergenomecompare/5.cross_sample/raw/"
    DIST_PATH="/data/user/yangzz/mapping/fieldergenomecompare/5.cross_sample/raw/"
    ...
    for i in range(1, n_bin+1):  #[)
        ...
        if i%BINSIZE==0:
            ...
            np.savetxt(CHR+"."+str(count)+".1M.dist.txt", init_mat, "%i")
            init_mat=np.zeros((n_sample, n_sample))
            count+=1
    if i%BINSIZE!=0:
