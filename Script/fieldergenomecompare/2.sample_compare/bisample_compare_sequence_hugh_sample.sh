           ...
           for num in {1..7}; do
             for i in {A,B,D}; do
               for item in {deletion,duplication};do
                 ...
                 if [ "${b}" == 0 ];then
                   #echo 'file zero' 
                   awk '{print $1"\t"$2"\t"$3"\t"SAMPLE2"'"$item"'""_CNV"}' SAMPLE2="$SAMPLE2" ${SAMPLE2}_${time_date}/chr${num}${i}.mask_CNV_${item}  > ${SAMPLE1}_${SAMPLE2}/chr${num}${i}_${SAMPLE2}to${SAMPLE1}_own_CNV_${item}
                 else
                   #echo 'file not zero' 
                   awk 'NR==FNR{a[$2]=$0;next}NR>FNR{if($2 in a ==0) print $1"\t"$2"\t"$3"\t"SAMPLE2"'"$item"'""_CNV"}' SAMPLE2="$SAMPLE2" ${SAMPLE1}_${time_date}/chr${num}${i}.mask_CNV_${item} ${SAMPLE2}_${time_date}/chr${num}${i}.mask_CNV_${item} > ${SAMPLE1}_${SAMPLE2}/chr${num}${i}_${SAMPLE2}to${SAMPLE1}_own_CNV_${item}
                 fi
        #
                 ...
                 if [ "${b}" == 0 ];then
                    #echo 'file zero'
                    awk '{print $1"\t"$2"\t"$3"\t"SAMPLE1"'"$item"'""_CNV"}' SAMPLE1="$SAMPLE1" ${SAMPLE1}_${time_date}/chr${num}${i}.mask_CNV_${item}  > ${SAMPLE1}_${SAMPLE2}/chr${num}${i}_${SAMPLE1}to${SAMPLE2}_own_CNV_${item}
                 else
                    #echo 'file not zero'
                    awk 'NR==FNR{a[$2]=$0;next}NR>FNR{if($2 in a ==0) print $1"\t"$2"\t"$3"\t"SAMPLE1"'"$item"'""_CNV"}' SAMPLE1="$SAMPLE1" ${SAMPLE2}_${time_date}/chr${num}${i}.mask_CNV_${item} ${SAMPLE1}_${time_date}/chr${num}${i}.mask_CNV_${item} > ${SAMPLE1}_${SAMPLE2}/chr${num}${i}_${SAMPLE1}to${SAMPLE2}_own_CNV_${item}
                 fi
                 awk 'NR==FNR{a[$2]=$0;next}NR>FNR{if($2 in a ==1) print $1"\t"$2"\t"$3"\t""'"$item"'""_both_CNV"}' ${SAMPLE1}_${time_date}/chr${num}${i}.mask_CNV_${item} ${SAMPLE2}_${time_date}/chr${num}${i}.mask_CNV_${item} > ${SAMPLE1}_${SAMPLE2}/chr${num}${i}_${SAMPLE1}to${SAMPLE2}both_CNV_${item}
            #3  
