...
(while read line;do
  gawk 'NR>1{if($1<0.5 || $1>1.5){print 0}else{print 1}}' ~/mapping/fieldergenomecompare/20200418_shannong/${line}/${CHR}.1M.norm | /data2/rawdata2/bin/datamash-1.4/datamash transpose
done < metadata_sample_list) | /data2/rawdata2/bin/datamash-1.4/datamash transpose > ${CHR}.CNVfilter.txt
done
