...
SAMPLE2=`echo $line|awk -F ',' '{print $2}'`
    ...
    awk -F '\t' 'NR==FNR{a[$1$2]=$0;next}NR>FNR{if ($1$2 in a==1) {print a[$1$2]} else {print $0}}' ${SAMPLE1}_${SAMPLE2}/${chr}.${SAMPLE1}to${SAMPLE2}all_CNV ${SAMPLE1}_${SAMPLE2}/${chr}.only_homo_snp_undefined_level > ${SAMPLE1}_${SAMPLE2}/${chr}.homo_undefined_snp_level    
    fi
