...
for chr in chr1A chr1B chr1D chr2A chr2B chr2D chr3A chr3B chr3D chr4A chr4B chr4D chr5A chr5B chr5D chr6A chr6B chr6D chr7A chr7B chr7D;do
    gawk '{sum+=$1} (NR%10)==0{print sum/10; sum=0} END{n=(NR%10);if(n!=0){print sum/n}}' ${SM}_2020-04-17162314/${chr}.1.1M.tmp > ${SM}_2020-04-17162314/${chr}.1.10M.tmp
    gawk '{sum+=$1} (NR%10)==0{print sum/10; sum=0} END{n=(NR%10);if(n!=0){print sum/n}}' ${SM}_2020-04-17162314/${chr}.2.1M.tmp > ${SM}_2020-04-17162314/${chr}.2.10M.tmp
