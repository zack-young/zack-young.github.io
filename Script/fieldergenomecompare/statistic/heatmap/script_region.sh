...
while getopts 'c:s:e:a:f:n:p:u:' opt
do
    case "$opt" in
    c) CHR=$OPTARG ;;
    s) START=$OPTARG ;;
...
tmpfileS=`mktemp -p /data/user/yangzz/mapping/fieldergenomecompare/statistic/heatmap/`
tmpfileG=`mktemp -p /data/user/yangzz/mapping/fieldergenomecompare/statistic/heatmap/`
for ((i=1; i<=$#; i++)); do
    #printf "%d\t%s\n" $i "${!i}"
...
tmpfile=`mktemp -p /data/user/yangzz/mapping/fieldergenomecompare/statistic/heatmap/`
...
# -v snp \  -S $tmpfileS \  -q 0.01:minor \ --min-ac=1 -M2 -m2 \
#echo "" >> ${GENE}.gene
#echo "# "$GENE >> ${GENE}.gene
#echo "# "$ANNO >> ${GENE}.gene
#bcftools view /data2/rawdata2/mergeFile/version2/${CHR}.ann.bcf.gz \
...
Rscript PlotHaplotype.R -i $tmpfile -g $tmpfileG -a $LABEL -n "n" -r ${CHR}:${START}-${END} -o ${CHR}_${START}_${END}_${suffix}
