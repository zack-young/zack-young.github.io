...
while read line;do
           #myjob=`jobs|wc -l`
           SAMPLE1=`echo $line | awk -F '_' '{print $1}'`
           SAMPLE2=`echo $line | awk -F '_' '{print $2}'`
           #num_low=0
           #num_diff=0
           #num_cnv=10
           #echo $num_tmp
           item=`awk '{if($2==num) print $4}' num="$START" ${dev_path}/${line}/${chr}.${suffix}`
           if [[ "$item" = "low" ]];then
             num_tmp=$(($num_tmp + 1))
...
wait
echo -e "$START\t$num_tmp" >> ${chr}_three_sample.txt
