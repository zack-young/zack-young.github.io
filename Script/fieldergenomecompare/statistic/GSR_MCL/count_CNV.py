...
def Count(infile, combine_data, count, compensent, normalize, split, chrom, pop_num, compensent_duplication):
    # open the infile
    try:
        if infile:
    ...
    if count == 'on' :
        for line in IN:
            ...
            dic1[line[0]] = dic1.setdefault(line[0], 0)+1
        for key,value in dic1.items():
            print('{key}\t{value}'.format(key=key, value=value), sep='\t')  #value=value/int(pop_num))
#
    if normalize == 'on':
        lis=[]
        for line in IN:
