...
Plink=/data/user/yangzz/worktools/Plink
for maf in 01;do
for miss in 05 1 2 3 4 5;do
(for CHR in AABBDD;do
  ${Plink}/plink --bfile ${CHR}_snp --maf 0.${maf} --geno 0.${miss} --make-bed --allow-extra-chr --out ${CHR}_setname_maf${maf}_miss${miss}
  ${Plink}/plink --bfile ${CHR}_setname_maf${maf}_miss${miss} --distance square 1-ibs flat-missing --out ${CHR}_setname_maf${maf}_miss${miss} --allow-extra-chr
