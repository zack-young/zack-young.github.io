...
def Graphmaker(pathway, sample, num):
    ...
    for i in range(6, -1, -1):
        ...
        for j in range(2, -1, -1):
            #print(pathway + sample +"_chr_graphy"+ "/"+ 'chr' + str(i+1) + lett[j] + ".data"+ "\tchr" + str(i+1) + lett[j])
            if num == 1: 
                print(str(pathway[0]) + "/" + 'chr' + str(i+1) + lett[j] + str(sample[0]) + "\tchr" + str(i+1) + lett[j]+ "\tNA")
            else:
                print(str(pathway[0]) + "/" + 'chr' + str(i+1) + lett[j] + str(sample[0]) + "\tchr" + str(i+1) + lett[j]+ "\tNA")
                for item in range(1,num):
                    print(str(pathway[item]) + "/" + 'chr' + str(i+1) + lett[j] + str(sample[item]) + "\tNA"+"\t"+str(item))
            print("NA\tNA\tNA")
