...
set -e
cd $1
for CHR in `samtools view -H ${WP}/04.*/$1/aa.filter.bam | grep '^@SQ' | gawk '{print substr($2, 4)}' | grep '\.1'` Mt; do
  (echo $CHR ;
  #
   ( samtools view -H ${WP}/04.*/$1/aa.filter.bam;
  for BAM in ${WP}/04.*/$1/??.filter.bam ; do samtools view ${BAM} $CHR; done ) | samtools view -Sb - > ${CHR}.bam
  #
...
for CHR in `samtools view -H ${WP}/04.*/$1/aa.filter.bam | grep '^@SQ' | gawk '{print substr($2, 4)}' | grep '\.2'` Pt; do
  (echo $CHR ;
  #
   ( samtools view -H ${WP}/04.*/$1/aa.filter.bam;
  for BAM in ${WP}/04.*/$1/??.filter.bam ; do samtools view ${BAM} $CHR; done ) | samtools view -Sb - > ${CHR}.bam
