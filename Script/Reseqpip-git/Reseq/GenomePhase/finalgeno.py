    ...
    for line in IN :
        ...
        if IsGeno(g1) and IsGeno(g2) and IsGeno(gs):
            while pos > end :
                ...
                if unconsis >= 2000 :
                    if ratio > 0.664 :
                        print("\t".join([b, "%s" % start, "%s" % end, "%s" % count, "NA", "NA", "highthree"]))
                    else:
                        OutputLinehomohigh(b, start, end, count, count1, count2)
                elif 100 <= unconsis <2000 :
                    if ratio > 0.664 :
                        print("\t".join([b, "%s" % start, "%s" % end, "%s" % count, "NA", "NA", "midthree"]))
                    else:
