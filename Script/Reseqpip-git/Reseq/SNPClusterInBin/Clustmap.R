...
option_list <- list(
    ...
    make_option(c("-k", "--kinds"), dest = "kinds", default = 10,
                            help = "[opt] kinds of main sample. [default: 10]"),
...
for(i in 1:ncol(data)){
    ...
    for(j in 1:ncol(data)){
        ...
        sam2 <- data[,j]
        for(k in 1:length(sam1)){
            if(sam1[k] == sam2[k] && sam1[k] != 0){
                count <- count+1
            }
        }
    }
    same_count_value <- c(same_count_value, count)
}
same_count_header <- names(data)
#print(same_count_header)
loop_swi <- TRUE
while(loop_swi){
    loop_swi <- FALSE
    for(i in 1:(length(same_count_value)-1)){
        if(same_count_value[i] < same_count_value[i+1]){
            temp <- same_count_value[i]
            same_count_value[i] <- same_count_value[i+1]
            same_count_value[i+1] <- temp
            temp <- same_count_header[i]
            same_count_header[i] <- same_count_header[i+1]
            same_count_header[i+1] <- temp
            loop_swi <- TRUE
        }
    }
}
...
#Finding main samples
main_sample <- c()
used_group <- c()
#print(banch)
print(length(banch))
while(length(main_sample) != arguments$kinds){
    for(i in 1:length(same_count_header)){
        for(j in 1:length(banch)){
            if(names(banch)[j] == same_count_header[i] && !(banch[[j]] %in% used_group)){
                main_sample <- c(main_sample, names(banch[j]))
                used_group <- c(used_group, banch[[j]])
                break
            }
        }
    }
...
for(j in 1:length(main_sample)){
    for(i in 1:length(names(data))){
        if(main_sample[j] == names(data)[i]){
            main_sample_id <- c(main_sample_id, i)
            break
        }
    }
}
print(main_sample)
print(main_sample_id)
for(i in 1:nrow(data)){
    add <- c()
    main_sample_value=c()

    for(j in 1:length(main_sample_id)){
        main_sample_value <- c(main_sample_value, data[i, main_sample_id[j]])
    }

    for(j in 1:ncol(data)){
        swi <- TRUE
        for(m in 1:length(main_sample_value)){
            if(data[i,j] == 0){
                swi <- FALSE
                add <- c(add, 1)
                break
            ...
            if(data[i,j] == main_sample_value[m] && data[i,j] != 0){
                swi <- FALSE
                add <- c(add, m+1)
                break
            }
    ...
    if(i == nrow(data)){
        data <- rbind(data[1:(nrow(data)-1), ], add)
    }else if(i == 1){
        data <- rbind(add, data[(i+1):nrow(data), ])
    }else{
        data <- rbind(data[1:(i-1), ], add, data[(i+1):nrow(data), ])
    }
}
...
for(i in 1:ncol(data)){
	...
	for (j in 1:length(data[,i])){
		if(as.numeric(data[j,i]+1) > length(color_pad)){
			color <- c(color, "#000000")
		}else{
			color <- c(color, color_pad[as.numeric(data[j,i]+1)])
		}
	}
...
string_otp <- paste(main_sample, collapse=" ")
