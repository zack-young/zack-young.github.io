...
def analyze(chr_cur, start, end, bin_size, sample_matrix, debug):
    if int(debug) > 1:
        print("[*] call analyze")
    #
...
def Cluster ( infile, bin_size = 1000000,
            ...
            if chr == chr_cur :
                while pos > end :
                    if int(debug) > 1:
                        ...
                        print ("[loop]", "001")
                        print ("========================================")
                    analyze(chr_cur, start, end, bin_size, sample_matrix, debug)
                    start += bin_size
                    end += bin_size
                    sample_matrix = [ [] for i in range(0,len(sample_header))]
                #
...
def main():
    ...
    if (options.outfile is not None) :
        ...
        #
    #
    Cluster (options.infile, int(options.bin_size), 
