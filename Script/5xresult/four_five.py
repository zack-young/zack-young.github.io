...
def GenoPhase(infile, bin_size, highun, lowun, hetedivision, undefinedivision,
    ...
    for line in IN:
        ...
        while pos > end:
            male = 2*count1 + hete
            female = 2*count2 + hete
            if male > 2300 and female <= 3500:
		pass
            elif male <= 2300 and female > 3500 :
		pass
            elif male > 2300 and female > 3500:
		pass
            else:
                print("\t".join([b, "%s" % start, "%s" % end, "%s" % count,"%s" % count1, "%s" % count2, "%s" % hete]))
            # if unconsis >= highun:
            #     print("\t".join([b, "%s" % start, "%s" % end, "%s" % count, "NA", "NA", "high"]))
            # elif lowun <= unconsis < highun:
            #     print("\t".join([b, "%s" % start, "%s" % end, "%s" % count, "NA", "NA", "mid"]))
            # else:
            #     print("\t".join([b, "%s" % start, "%s" % end, "%s" % count, "NA", "NA", "low"]))
            # #
            # if ratio1 > hetedivision:
            #     print("\t".join([b + '.1', "%s" % start, "%s" % end, "%s" % count, "NA", "NA", "hete"]))
            # else:
            #     OutputLinehomohigh(b + '.1', start, end, count, count1, count2)
            #print("\t".join([b + '.2' , "%s" % start, "%s" % end, "%s" % count, "NA", "NA", "NA"]))
            start += bin_size
            end += bin_size
            count = 0
            count1 = 0
        ...
        if IsGeno(gs):
            ...
            undefinedson += 1
        if IsGeno(g1) and IsGeno(g2) and IsGeno(gs) and IsHomo(gs) and IsHomo(g1) and IsHomo(g2):
