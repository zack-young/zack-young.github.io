...
def GenoPhase(infile, bin_size, highun, lowun, hetedivision, undefinedivision,
    ...
    for line in IN:
        ...
        b = chr_cur[0]
        while pos > end:
            axis = math.log10(2*father + hete + 1)
            ayis = math.log10(2*mother + hete + 1)
            if son >= highun and count2 >= highun and count3 >= highun:
                compare(b, start, end, hete, father, mother, axis, ayis)
            elif lowun <= son < highun and lowun <= count2 < highun and lowun <= count3 < highun:
                compare(b, start, end, hete, father, mother, axis, ayis)
            elif son < lowun and count2 < lowun and count3 < lowun:
                compare(b, start, end, hete, father, mother, axis, ayis)
            else:
                print("\t".join([b, "%s" % start, "%s" % end, "6", "%s" % hete, "%s" % father, "%s" % mother]))
            # #
            start += bin_size
            end += bin_size
            count1 = 0
