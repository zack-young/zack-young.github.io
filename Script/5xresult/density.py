...
def GenoPhase(infile, bin_size, highun, lowun, hetedivision, undefinedivision,
    ...
    for line in IN:
        ...
        while pos > end:
            ...
            print("\t".join([b, "%s" % start, "%s" % end, "%s" % count1]))
#            if count1 >= highun:
#                print("\t".join([b, "%s" % start, "%s" % end, "high"]))
#            elif lowun <= count1 < highun:
#                print("\t".join([b, "%s" % start, "%s" % end, "mid"]))
#            else:
#                print("\t".join([b, "%s" % start, "%s" % end, "low"]))
            #
...
def main():
    ...
    GenoPhase(options.infile, int(options.bin_size), int(options.highun), int(options.lowun), 
              ...
              options.chr_col, options.pos_col, int(options.s_col))
    #
#
