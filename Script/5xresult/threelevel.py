...
def GenoPhase(infile, bin_size, highun, lowun, hetedivision, undefinedivision,
    ...
    for line in IN:
        ...
        while pos > end:
            ...
            if unconsis >= highun:
                print("\t".join([b, "%s" % start, "%s" % end, "%s" % count, "%s" % count1, "%s" % count2, "%s" % hete, "high"]))
            elif lowun <= unconsis < highun:
                print("\t".join([b, "%s" % start, "%s" % end, "%s" % count, "%s" % count1, "%s" % count2, "%s" % hete, "mid"]))
            else:
                print("\t".join([b, "%s" % start, "%s" % end, "%s" % count, "%s" % count1, "%s" % count2, "%s" % hete, "low"]))
            # #
            start += bin_size
            end += bin_size
            count = 0
