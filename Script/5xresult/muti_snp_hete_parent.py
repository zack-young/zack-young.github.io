...
def GenoPhase(infile, bin_size, gene_snp_filter, chrom_graphy, 
              DP_density, snp_graphy, mask_DP, long_short_count, hete_ratio_count,
...
#    Makedic(sample)
    ...
    if chrom_graphy == 'on':   # find discard bin
        ...
        #
    #
    if DP_density == 'on':   # counting DP per bin
        firstline = IN.readline().strip()
        item = firstline.strip().split("\t")
        firstpos = int(item[pos_col - 1])
        if int(firstpos) <= int(bin_size):
        ...
        if judge==0:
            if count3 == 0:
                ...
                DP_ratio = DP_count/count3
            print("\t".join([b,"%s" % start, "%s" % end, "%.3f" % DP_ratio]))
        #if float(dic1[sample][(num)*3+ch][count]) >= 0.6:
        #    print("\t".join([b,"%s" % start, "%s" % end, "%.3f" % DP_ratio,"%s" % count3]))
    #
    ...
    if filter_and_count_snp == 'on':  # filter on sample snp and count homo_alt snp 
        ...
        if float(dic1[sample][(num)*3+ch][count]) < 0.5:
            ...
            print("\t".join([b, "%s" % start, "%s" % end, "%s" % count1, "%s" % count2]))
    #
    if hete_ratio_count == 'on': # level differnt snp number between two samples with alt genotype
        firstline = IN.readline().strip()
        item = firstline.strip().split("\t")
        firstpos = int(item[pos_col - 1])
        if int(firstpos) <= int(bin_size):
...
def main():
    ...
    parser.add_option("--chrom_graphy", dest="chrom_graphy",
                  help="column id for offSpring [default: %default]", metavar="STRING",
                      default = 'off')
    parser.add_option("--DP_density", dest="DP_density",
                  help="column id for offSpring [default: %default]", metavar="STRING",
                      default = 'off')
    ...
    parser.add_option("--test", dest="test",
                  help="column id for offSpring [default: %default]", metavar="STRING",
                      default = 'off')   
    parser.add_option("--hete_ratio_count", dest="hete_ratio_count",
                  help="column id for offSpring [default: %default]", metavar="STRING",
                      default = 'off')
    ...
    GenoPhase(infile=options.infile, bin_size=int(options.bin_size), gene_snp_filter=options.gene_snp_filter, 
              chrom_graphy=options.chrom_graphy, DP_density=options.DP_density, snp_graphy=options.snp_graphy, 
              ...
              sample=options.sample, chromosome=options.chromosome,hete_ratio_count=options.hete_ratio_count, 
