library(gplots)
files = read.table("/data/user/yangzz/mapping/fieldergenomecompare/statistic/NongDa5181/plotfile_nd5181_2p", as.is = T, header = F, comment.char = "")
options(scipen=200)
...
#for(j in 1:12){
  if(!is.na(files[j,1])){
    ...
    if(file.exists(files[j,1]) == 0) {
      ...
      #      )
      chro1 <- factor(chro, 
                      levels = c("NZ05","YM03","S140_unknown","YM03_NZ05_both"),
      ...
      if(!is.na(files[j,2])){
        ...
        if (files[j,3]==1){
          text(x=len, y=j, "SGR with Nongda3097", cex = 0.5 ,pos=4)
        ...
        if (files[j,3]==2){
          ...
          text(x=len, y=j-0.45, "SGR with Lunxuan987", cex = 0.5 ,pos=4)
        }
