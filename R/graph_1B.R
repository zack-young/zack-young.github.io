library(gplots)
files = read.table("/data/user/yangzz/mapping/fieldergenomecompare/statistic/1B1R_lovrin/plotfile_1B1R_10_26", as.is = T, header = F, comment.char = "")
...
color_pad <- c(low1,low2,low3,low2,delboth,dupboth,undefined)
...
text(x=rep(860,9), y=seq(18,42,3),#x=seq(-190,-50,20), y=rep(30.5,8),
     c("Undefiend","884187","954072","jinan17","Both Deletion","Both Duplication","HMW","LMW","Gliadin"),cex = 0.8,pos=4)#,srt=90)
#"884187","954072","jinan17"ji200040919
rect(xleft = rep(850,6), xright = rep(860,6),
     ybottom = seq(17.5,33.5,3),
     ytop = seq(18.5,34.5,3),
     col = c(undefined,low3,low2,low1,delboth,dupboth), border = NA)
...
for(j in 1:nrow(files)){
  if(!is.na(files[j,2])){
  text(x=-80, y=j-1.1, files[j,2], cex = 1)#,srt = 90)
  }
  if(!is.na(files[j,3])){
    if(file.exists(files[j,1]) != 0) {
      ...
      #      )
      chro1 <- factor(chro, 
                      levels = c("low0","low1","low3","low4",'deletion_both_CNV','duplication_both_CNV',"undefined"),
    ...
    plotChrom(0, j-1.7, 1.4, len, centro, 1000000)
  }
