...
options(scipen=200)
#files = read.table(paste(DEV,"/plotfile/","plotfile_",SAMPLE1,sep=""), as.is = T, header = F, comment.char = "")
files=read.table("/data/user/yangzz/mapping/fieldergenomecompare/20200416_201_CNV/plotfile/plotfile_S212", as.is = T, header = F, comment.char = "")
pdf(paste(DEV,"/pdf/",SAMPLE1_name,suffix,".pdf",sep=""), height = figure.height, width = figure.width)
...
plot(x=0, type="n", bty="n", yaxt="n",xaxt="n",
     ...
     xaxs="i", yaxs="i", 
     #main=paste("S",j,sep=""),
     main=paste(SAMPLE1_name,"CNV Distribution",sep=" "),
     cex.main = 2.5)
...
rect(xleft = 900, xright = 930, ybottom = 16, ytop = 18,
     col = color_pad[1], border = color_pad[4]);
text(x=c(940), y=c(17), paste("deletion",sep=""),cex = 1,pos = 4)
#rect(xleft = 900, xright = 930, ybottom = 30, ytop = 32,
#     col = color_pad[6], border = color_pad[6]);text(x=c(940), y=c(31),c("Both_deletion_CNV"),cex = 1,pos = 4)
rect(xleft = 900, xright = 930, ybottom = 20, ytop = 22,
     col = color_pad[2], border = color_pad[5]);
text(x=c(940), y=c(21),paste("duplication",sep=""),cex = 1,pos = 4)
  ...
  if(!is.na(files[j,1])){
    ...
    if(file.info(files[j,1])$size!=0){
      ...
      chro1 <- factor(chro, 
                      ...
                      #
                      levels = c("ownCNV_deletion","ownCNV_duplication"),
