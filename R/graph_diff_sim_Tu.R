...
options(scipen=200)
files = read.table(paste("/data/user/yangzz/mapping/S15/ReseqPipeNew/09.filterVCF/GT_AD/plotfile/plotfile_",SAMPLE1,"_",SAMPLE2,sep=""), as.is = T, header = F, comment.char = "")

pdf(paste("/data/user/yangzz/mapping/S15/ReseqPipeNew/09.filterVCF/GT_AD/pdf/",SAMPLE1_name,"_",SAMPLE2_name,arguments$suffix,".pdf",sep=""), height = figure.height, width = figure.width)
par(mar=c(5, 5, 5, 5))
...
color_pad <- c('#9FC91D',midblue,red,'white','white','white')
num = 22
...
rect(xleft = 900, xright = 930, ybottom = nrow(files)/2, ytop = nrow(files)/2+1,
     col = color_pad[3], border = color_pad[3]);text(x=c(940), y=c( nrow(files)/2+0.5),c("LOW_Diff"),cex = 1,pos = 4)
rect(xleft = 900, xright = 930, ybottom = nrow(files)/2+1, ytop = nrow(files)/2+2,
     col = color_pad[2], border = color_pad[2]);text(x=c(940), y=c( nrow(files)/2+1.5),c("MID_Diff"),cex = 1,pos = 4)
rect(xleft = 900, xright = 930, ybottom = nrow(files)/2+2, ytop = nrow(files)/2+3,
     col = color_pad[1], border = color_pad[1]);text(x=c(940), y=c( nrow(files)/2+2.5),c("HIGH_Diff"),cex = 1,pos = 4)
