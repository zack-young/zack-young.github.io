...
#files = read.table("/data/user/yangzz/mapping/fieldergenomecompare/statistic/GSR_MCL/CNCsample_2_CNL_FC.txt", as.is = T, header = F, comment.char = "")
...
aql_all$variable <- factor(aql_all$variable,levels = c("OTH", 'line_CNC',"line_FC_CNL","line_CNL","line_FC"))
ggplot(aql_all)+
  geom_bar(aes(x = sample, y = value/14075,alpha = 1/10),stat = "identity",position ='stack')+ 
  ...
  theme(panel.grid = element_blank(), panel.border = element_blank(),          
        ...
        axis.title.x=element_text(size=1,color="black",angle=45),
...
ggplot(aql_all,aes(x=variable,y=value/14075)) +
  geom_violin(width = 0.75,colour = "black",fill="#17A3C0")+
  ...
  scale_x_discrete(labels=c("Foreign Cultivars","Chinese Landrace"))+
  scale_y_continuous(limits = c(0,0.7) ,expand = c(0,0))+ylab('Proportion')
